...
void		Game::run()
{
	...
	usq = 0;
	ust = 0;
	Game::console->setLevel(1);
	Game::console->setScore(0);
	enemyShoot = 0;
...
void 		Game::loop(void) {
	...
	if (!this->player->loop())
		this->endGame();
	for (int i = 0; i < NB_ENEMIES ; ++i)
		this->enemies[i].loop(ust, enemyShoot);
	...
	usq = (usq == 4) ? 0 : usq + 1;
	ust = (ust == 3) ? 0 : ust + 1;
	enemyShoot = (enemyShoot == 140) ? 0 : enemyShoot + 1;
	for (int i = 0; i < NB_ASTEROIDS; ++i)
	{
...
void 		Game::aff( void ) {
	...
	attron(COLOR_PAIR(ASTEROID));
	for (int i = 0; i < NB_ASTEROIDS; ++i)
			this->asteroids[i].display();
	attroff(COLOR_PAIR(ASTEROID));
