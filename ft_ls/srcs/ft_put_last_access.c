...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_put_last_access.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: kbunel <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/06/20 14:47:17 by kbunel            #+#    #+#             */
...
static void		check_ascii(t_file *file, t_file *g_file, int i)
{
	...
	{
		if (g_file->prev != NULL)
			ft_put_last_access(file, file->parent, g_file->prev, -1);
		else
			g_file->prev = file;
	}
	else if (file->name[i] > g_file->name[i])
	{
		if (g_file->next != NULL)
			ft_put_last_access(file, file->parent, g_file->next, -1);
		else
			g_file->next = file;
	}
}
		...
		t_file *g_file, int i)
{
	...
	{
		if (g_file->prev != NULL)
			ft_put_last_access(file, parent, g_file->prev, -1);
		else
			g_file->prev = file;
	}
	else if (i == -1 && file->natimespec < g_file->natimespec)
	{
		if (g_file->next != NULL)
			ft_put_last_access(file, parent, g_file->next, -1);
		else
			g_file->next = file;
	}
}
